...
class DeploygkeWorkflowIntegrationTest(test_base.DjangoFileGeneratorTest):
  ...
  def test_deploygke_workflow(self):
    ...
    with self.clean_up_cluster(cluster_name), \
      ...
      # It has access to manage all resources. Because generation of service
      # account key is not part of this test case, I just reuse this existing
      ...
      admin_url = self.deploygke_workflow.deploygke(
          service_account_key_path, 'fake_admin_user',
          cluster_name=cluster_name, bucket_name=bucket_name,
          image_tag=self.image_tag, open_browser=False)
