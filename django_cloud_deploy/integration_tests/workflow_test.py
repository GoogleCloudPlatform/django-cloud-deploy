...
class EnableServiceWorkflowIntegrationTest(test_base.BaseTest):
  ...
  def setUp(self):
    ...
    enable_service_client = (
        enable_service.EnableServiceClient.from_credentials(self.credentials))
...
class ServiceAccountKeyGenerationWorkflowIntegrationTest(test_base.BaseTest):
  ...
  ROLES = (
      'roles/cloudsql.client', 'roles/cloudsql.editor', 'roles/cloudsql.admin'
  )

  KEY_EXPECTED_ATTRIBUTES = (
  ...
  def setUp(self):
    ...
    self.service_account_workflow = (
        _service_account.ServiceAccountKeyGenerationWorkflow(self.credentials))
  ...
  def test_create_service_account_key(self):
    ...
    with self.delete_service_account(self.project_id, service_account_email):
...
class DeploygkeWorkflowIntegrationTest(test_base.DjangoFileGeneratorTest):
  ...
  def setUp(self):
    ...
    self.deploygke_workflow = (
        _deploygke.DeploygkeWorkflow(self.credentials))
  ...
  def clean_up_cluster(self, cluster_name):
    try:
      ...
      request = self.container_service.projects().zones().clusters().delete(
    ...
    with self.clean_up_cluster(cluster_name):
      with self.clean_up_docker_image(self.image_tag):
        admin_url = self.deploygke_workflow.deploy_new_app_sync(
            project_id=self.project_id, cluster_name=cluster_name,
            app_directory=self.project_dir, app_name=self.project_name,
            image_name=self.image_tag, secrets=secrets)
