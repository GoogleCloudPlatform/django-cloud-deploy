...
class _Jinja2FileGenerator(_FileGenerator):
  ...
  def _render_directory(self, template_dir, output_dir,
...
class _DjangoFileGenerator(_Jinja2FileGenerator):
  ...
  def generate_project_files(self, project_id, site, destination):
    ...
    project_templates_dir = os.path.join(
        self._get_template_folder_path(), self.PROJECT_TEMPLATE_FOLDER)
    options = {
  ...
  def generate_app_files(self, app_name, destination,
    ...
    app_templates_dir = os.path.join(
        self._get_template_folder_path(), template_folder)
  ...
  def generate_admin_files(self, destination):
    self.generate_app_files('cloud_admin',
                            destination,
...
class _DockerfileGenerator(_Jinja2FileGenerator):
  ...
  def generate(self, project_name, destination):
    ...
    for file_name in file_names:
      template_path = os.path.join(self._get_template_folder_path(), file_name)
...
class _YAMLFileGenerator(_Jinja2FileGenerator):
  ...
  def generate(self, destination, project_name, project_id, instance_name=None,
               region='us-west1', image_tag=None):
    """Generate YAML file which defines Kubernete deployment and service.
...
class DjangoSourceFileGenerator(_FileGenerator):
  ...
  def _generate_django_source_files(self, project_id, project_name, app_names,
                                    destination):
    self.django_file_generator.generate_project_files(project_id, project_name,
                                                      destination)
  ...
  def generate_all_source_files(self, project_id, project_name, app_names,
                                destination, instance_name=None,
                                region='us-west1', image_tag=None):
