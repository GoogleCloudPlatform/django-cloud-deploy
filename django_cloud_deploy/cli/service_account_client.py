...
class ServiceAccountClient(base_client.BaseClient):
  ...
  def _create_service_account(self, service_account_name=None):
    ...
    subprocess.check_call(['gcloud', 'iam', 'service-accounts', 'create',
                           service_account_name, '--display-name',
                           service_account_name], stdout=self._stdout,
                          stderr=self._stderr)
  ...
  def _grant_roles(self, service_account_name=None):
    ...
    for role in roles:
      subprocess.check_call(['gcloud', 'projects', 'add-iam-policy-binding',
                             ...
                             'serviceAccount:' + service_account_id, '--role',
                             role], stdout=self._stdout,
                            stderr=self._stderr)
  ...
  def _download_keys(self, service_account_name=None):
    ...
    subprocess.check_call(['gcloud', 'iam', 'service-accounts', 'keys',
                           'create', key_path, '--iam-account',
                           service_account_id], stdout=self._stdout,
                          stderr=self._stderr)
    return key_path
