...
class DatabaseWorkflow(base_client.BaseClient):
  ...
  def __init__(self, io: workflow_io.IO, project_id: str, project_name: str,
               project_dir: str, database_client: database.DatabaseClient,
               cloud_sql_proxy_path: str = 'cloud_sql_proxy',
               zone: str = 'us-west1', debug=False):
  ...
  def create_database_workflow(self):
    ...
    self.database_client.create_instance_sync(self.project_id, instance_name)
    self.database_client.create_database_sync(self.project_id, instance_name,
                                              db_name)
    self.database_client.set_database_password(self.project_id, instance_name,
                                               'postgres', self._password)
  ...
  def create_superuser_workflow(self):
    ...
    # 'createsuperuser' command. However, because we are running django-admin
    # in a subprocess instead of a shell, we are not able to provide input to
    # that command. So we have to firstly create a superuser and set password
    # afterwards.
    # See https://github.com/django/django/blob/master/django/contrib/auth/management/commands/createsuperuser.py#L80  # noqa: E501
    print('Creating a superuser for your Django admin site.\n'
    ...
    with self._with_cloud_sql_proxy():
      ...
      createsuperuser_args = [
          ...
          '='.join(['--username', self._superuser_name]),
          '='.join(['--email', self._superuser_email]),
          '='.join(['--pythonpath', self.project_dir]),
          '='.join(['--settings', settings_module]), '--noinput']
      ...
      changepassword_args = (
          ('django-admin changepassword {} --pythonpath={} '
           '--settings={}')).format(
               self._superuser_name, self.project_dir, settings_module)

      # 'changepassword' command spawns the following output:
      # Password: <type your password>
      # Password (again): <type your password>
      # If your password is too common, it will ask you to use another password
  ...
  def migrate_database_workflow(self):
    ...
    with self._with_cloud_sql_proxy():
      ...
      # "makemigrations" will generate migration files based on definitions in
      # models.py.
      makemigrations_args = ['django-admin', 'makemigrations',
                             '='.join(['--pythonpath', self.project_dir]),
                             '='.join(['--settings', settings_module])]
      subprocess.check_call(makemigrations_args)

      # "migrate" will modify cloud sql database.
      migrate_args = ['django-admin', 'migrate',
                      '='.join(['--pythonpath', self.project_dir]),
                      '='.join(['--settings', settings_module])]
  ...
  def _with_cloud_sql_proxy(self):
    try:
      ...
      process = pexpect.spawn(self.cloud_sql_proxy_path, args=[instance_flag])
      CLOUD_SQL_READY = 'Ready for new connections'
      process.expect(CLOUD_SQL_READY, timeout=5)  # To avoid race condition
      yield

    except pexpect.exceptions.TIMEOUT:
      raise CloudSqlProxyError('Cloud SQL Proxy was unable to start correctly')
    finally:
  ...
  def ask_for_superuser_name(self):
    ...
    self._superuser_name = self.workflow.ask(
        ('Provide superuser name for your Django admin site. This account is '
         'useful to manage content on your site. Use [{}] '
         'by pressing enter:').format(default_name),
        default_name, self._is_valid_superuser_name, error_msg)
  ...
  def ask_for_superuser_email(self):
    ...
    error_msg = ('Invalid email address. The format of email address should '
    ...
    self._superuser_email = self.workflow.ask(
        ('Provide superuser email for your Django admin site. Use [{}] '
         'by pressing enter:').format(default_email),
        default_email, self._is_valid_email, error_msg)
    return self._superuser_email

  def ask_for_password(self):
    self.workflow.tell('Provide password for postgres database. We will use '
