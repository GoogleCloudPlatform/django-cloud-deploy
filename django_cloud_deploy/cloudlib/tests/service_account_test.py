...
class ServiceAccountClientTestCase(absltest.TestCase):
  ...
  def test_create_service_account_success(self):
    ...
    all_service_accounts = (
        self._iam_service_fake.projects_fake.service_accounts_fake.
        service_accounts)
  ...
  def test_create_service_account_server_returns_invalid_policy(self):
    service_account_id = 'test_create_service_account_returns_invalid_policy'
  ...
  def test_create_service_account_set_iam_policy_fail(self):
    ...
    with mock.patch(
        ('django_gke.cloudlib.service_account.ServiceAccountClient.'
         '_get_iam_policy'), autoSpec=True
  ...
  def test_create_service_account_key_success(self):
    ...
    key = self._service_account_client.create_key(
        PROJECT_ID, service_account_id)

    self.assertDictEqual(
        key, json.loads(PRIVATE_KEY_DECRYPTED.decode('utf-8')))
  ...
  def test_create_service_account_key_failure(self):
    ...
    with self.assertRaises(service_account.ServiceAccountKeyCreationError):
      self._service_account_client.create_key(
          project_id, service_account_id)
