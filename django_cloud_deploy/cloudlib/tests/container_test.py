  ...
  def get(self, projectId, zone, clusterId):
    ...
    if clusterId not in self.clusters_to_get_count:
      ...
      if get_count < total_get_count:
        ...
        status = 'RUNNING'
    response = CLUSTER_GET_RESPONSE_TEMPLATE.format(
        clusterId, ca, status)
...
class LocationsFake(object):
  ...
  def getServerConfig(self, name):
    del name
    return http_fake.HttpRequestFake({'defaultClusterVersion': '1.9.7-gke.6'})
...
class ContainerClientTestCase(absltest.TestCase):
  ...
  def setUp(self):
    ...
    self._container_client = container.ContainerClient(self._container_service)
