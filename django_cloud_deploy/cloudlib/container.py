...
class ContainerClient(object):
  ...
  def __init__(self,
               container_service: discovery.Resource):
  ...
  def _get_default_kubernetes_version(self, project_id, region='us-west1'):
    ...
    request = self._container_service.projects().locations().getServerConfig(
        name=name)
  ...
  def create_cluster_sync(self, project_id: str, cluster_name: str,
                          region: str = 'us-west1', zone: str = 'us-west1-a'):
    ...
    try:
      ...
      if e.resp.status == 403:
        raise ContainerCreationError(
            ('You do not have permission to create a cluster in project: "{}"'
             .format(project_id)))
        ...
        raise ContainerCreationError(
            ('Unexpected error when creating cluster "{}" in project "{}"'
             .format(cluster_name, project_id))) from e
  ...
  def create_kubernetes_configuration(
    ...
    # credentials.token is a bearer token that can be used in HTTP headers to
    # make authenticated requests. When the given credentials does not have
    # token, we need to force it to get a new token.
    ...
    # Save temporary file path so that it can be cleaned up after the program
    # exits.
