...
class DeploygkeWorkflowTest(absltest.TestCase):
  ...
  @mock.patch('google.cloud.container_v1.ClusterManagerClient.create_cluster',
              autoSpec=True)
  @mock.patch('google.cloud.container_v1.ClusterManagerClient.get_cluster',
              autoSpec=True, return_value=MockGetClusterResponse())
  ...
  def _setup_django_files(self):
    ...
    project_folder_path = os.path.join(self._project_dir, self._project_name)
    os.mkdir(project_folder_path)

    settings_path = os.path.join(self._project_dir, self._project_name,
                                 'remote_settings.py')
    # This is used to find and collect static files for installed Django apps.
  ...
  def test_collect_static_files(self):
    ...
    files_list = os.listdir(os.path.join(self._project_dir, 'static', 'polls'))
  ...
  def _create_fake_responses_for_gcs_bucket_create(self, resp):
    ...
    resp.add(resp.POST, gcs_bucket_create_url, body=gcs_bucket_create_body,
  ...
  def _create_fake_responses_for_gcs_upload(self, resp):
    ...
    resp.add(resp.POST, gcs_upload_url, body=gcs_upload_response_body,
             content_type='application/json')
...
class GetIngressUrlTest(absltest.TestCase):
  ...
  def test_simple_success(self):
    with mock.patch(
        ...
        autoSpec=True
    ) as mock_list_service_for_all_namespaces:
      ...
      self.assertEqual(1, len(mock_list_service_for_all_namespaces.mock_calls))
  ...
  def test_success_on_second_call(self, sleep):
    with mock.patch(
      ...
      self.assertEqual(self.INGRESS_IP_URL,
                       deploygke.DeploygkeWorkflow._get_ingress_url())
      self.assertEqual(2, len(mock_list_service_for_all_namespaces.mock_calls))
...
class WaitForDeploymentReadyTest(absltest.TestCase):
  ...
  def test_simple_success(self):
    with mock.patch(
        ...
        autoSpec=True
    ) as mock_list_deployment_for_all_namespaces:
      ...
      self.assertEqual(self.READY_REPLICAS,
                       self._deploygke_workflow._wait_for_deployment_ready())
      self.assertEqual(1,
                       len(mock_list_deployment_for_all_namespaces.mock_calls))
  ...
  def test_success_on_second_call(self, unused_sleep):
    with mock.patch(
      ...
      mock_list_deployment_for_all_namespaces.side_effect = [
          (kubernetes.client.models.extensions_v1beta1_deployment_list.
           ExtensionsV1beta1DeploymentList(items=[self.DEPLOYMENT_NOT_READY])),
          (kubernetes.client.models.extensions_v1beta1_deployment_list.
