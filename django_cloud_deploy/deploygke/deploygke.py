...
class DeploygkeWorkflow(base_client.BaseClient):
  ...
  def __init__(self, project_id, project_name, project_path,
               storage_client=None, debug=False):
  ...
  def _collect_static_content(self):
    ...
    subprocess.check_call(['django-admin', 'collectstatic',
                           '='.join(['--pythonpath', self._project_path]),
  ...
  def _upload_static_content_recursive(self, bucket, current_path,
    ...
    for file_or_folder in os.listdir(current_folder_path):
      ...
      file_or_folder_path = os.path.join(current_folder_path, file_or_folder)
      if os.path.isdir(file_or_folder_path):

        # current_path and file_or_folder are both relative path with
        # the current folder.
        relative_folder_path = os.path.join(current_path, file_or_folder)
        self._upload_static_content_recursive(bucket, relative_folder_path,
                                              files_uploaded)
      else:
  ...
  def _wait_for_deployment_ready(self):
    ...
    print('Waiting for deployment to get ready.', sep='', end='', flush=True)
  ...
  def deploygke(self, service_account_key_path, admin_user_name,
                cluster_name=None, bucket_name=None, image_tag=None,
                open_browser=True):
  ...
  def _get_default_kubernetes_version(self, zone='us-west1-a'):
    ...
    output = subprocess.check_output(query_default_version_args,
                                     universal_newlines=True,
                                     stderr=self._stderr)
    return output.strip()

  def _create_cluster(self, cluster_name, region='us-west1',
                      zone='us-west1-a'):
    ...
    try:
      ...
      client.create_cluster(project_id=self._project_id, zone=zone,
                            cluster=cluster_data['cluster'])
      print('Waiting for cluster to get ready. This should take less than 5 '
            'minutes')
      time_to_wait = 300
      while time_to_wait > 0:
        response = client.get_cluster(self._project_id, zone, cluster_name)
        if response.status == 2:  # response.status = 2 means cluster is ready
          break
        else:
          time_to_wait -= 5
          time.sleep(5)
      ...
      print('Cluster {} already exists. We will reuse that cluster.'.format(
          cluster_name))
  ...
  def _create_service_and_deployment(self):
    ...
    print('Successfully created service. It should be ready within a minute.')
  ...
  def _update_service_and_deployment(self):
    ...
    subprocess.check_call(['kubectl', 'set', 'image', deployment_name,
                           set_image_str],
                          stdout=self._stdout, stderr=self._stderr)
    print('Successfully updated service. It should be ready within a minute.')
  ...
  def _configure_kubectl(self, cluster_name, zone='us-west1-a'):
    ...
    """
    subprocess.check_call(['gcloud', 'container', 'clusters',
                           'get-credentials', cluster_name, '--zone', zone],
                          stdout=self._stdout, stderr=self._stderr)
    kubernetes.config.load_kube_config()
  ...
  def _create_cloudsql_secrets(self, service_account_key_path):
    ...
    subprocess.check_call(['kubectl', 'create', 'secret', 'generic',
                          ...
                          stdout=self._stdout, stderr=self._stderr)
    subprocess.check_call(['kubectl', 'create', 'secret', 'generic',
                           'cloudsql', '--from-literal=username=' + username,
