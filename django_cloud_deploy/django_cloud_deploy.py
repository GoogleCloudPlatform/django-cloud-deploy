...
def _enable_apis(credentials, project_id):
  ...
  enable_service_workflow.enable_required_services(project_id=project_id,
...
def update(debug=False):
  ...
  db_workflow = database_workflow.DatabaseWorkflow(
      console, project_id, project_name, project_dir,
      cloud_sql_proxy_path=cloud_sql_proxy, database_client=None,
      debug=debug)
  ...
  deploygke_client = deploygke.DeploygkeWorkflow(project_id, project_name,
                                                 project_dir, debug=debug)
...
def new(debug=False):
  ...
  generator.generate_all_source_files(
      project_id, django_config.django_name, django_config.apps,
  ...
  db_workflow = database_workflow.DatabaseWorkflow(
      workflow_io.Console(), project_id, django_config.django_name,
      django_config.project_dir, cloud_sql_proxy_path=cloud_sql_proxy,
      database_client=database_client, debug=debug)
  ...
  deploygke_client = deploygke.DeploygkeWorkflow(
      project_id, django_config.django_name, django_config.project_dir,
      debug=debug)
